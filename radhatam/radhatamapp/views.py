...
def entity(request, action, id):
    ...
    if "GET" == request.method:
        ...
        return render(request, "radhatamapp/entity.html", context={'form': form, 'entity': entity, 'entities': entities, 'fields': fields, 'dtypes': DATA_TYPES, 'dataupload_form': dataupload_form})
...
def datascience(request, action, id):
    ...
    if "GET" == request.method:
        ...
        full_data_sql = generate_action_sql(data_sql, id, action) + '  order by trade_date desc limit 200'
        ...
        functions_calc_meta = FunctionMeta.objects.filter(type='CALCULATION')
        ...
        return render(request, "radhatamapp/datascience.html",
                      context={'entity': entity, 'fields': fields, 'data': data,
                               ...
                               'functions_calc_meta': functions_calc_meta,
                               'functions_viz_meta': functions_viz_meta,
                               'functions_ds_meta': functions_ds_meta,
                               'filters': filters,
    ...
    if action == 'add_derived_field':
        ...
        derived_field = Field.objects.create(
            actual_name=derived_field_name, name=derived_field_name, entity=entity, type='DERIVED', derived_level=level_field, function=function)
        derived_field.save()
